var WizzyRange = (function(){
	...
	logger = (function() {
		var methods = ["log", "debug", "dir", "info", "warn", "error", "group", "groupEnd"],
			len = methods.length, logs = {}, i, method;
		
		for(i=0; i<len; i++){
			method = methods[i];
			logs[method] = function(method){
				return function(){
					if(log && window.console && typeof(console[method]) === "function")
	...
	/**
	 * Class which wraps range. An EC Range instance is associated with WizzyRange.
	...
	WizzyRangeProto = {
		...
		setRange: function(r) {
			if(r) {
				...
				this.r = rantex.createRange();

				// IE Range				
				if(window.document.selection) {
					try {
						console.log("IE pre-process - is endContainer a textnode? ", this.isTNode('e'));

						var tmpr = fixIERangeObject(getRangeObject());
						//this.r.setEnd(tmpr.endContainer, tmpr.endOffset);
						this.r.endContainer = tmpr.endContainer;
						this.r.endOffset = tmpr.endOffset;
						console.log("IE post-process - now is endContainer a textnode? ", this.isTNode('e'));
					} catch(e) {
						console.log("Error from IE range", e);
					}
				}
			}
		...
		select: function() {
			return this.r.select();
		},


		/**
		 * Call rantex's range method
		 */
		...
		getCharInRange: function(which, offset) {
			...
			if(this.isTNode(which)) c = this.get(which).data.charAt(this.offset()[which] + offset);
			return c;
		},
		...
		trimRangeWhitespaces: function() {
			...
			if(this.isTNode('s')) {
				do {
					...
					i++; if(i==1000) { logger.log("Trim range loop broken at start"); break; }
				} while (nextSibling);
			}
			i=0;
			if(this.isTNode('e')) {
				do {
					...
					i++; if(i==1000) { logger.log("Trim range loop broken at end"); break; }
				} while (prevSibling);
			}
		},
		...
		includeRangeWhitespaces: function() {
			...
			if(this.isTNode('s')) {
				do {
					...
					i++; if(i==1000) { logger.log("Include range loop broken at start"); break; }
				} while (prevSibling);
			}
			i=0;
			if(this.isTNode('e')) {
				do {
					...
					// For infinite loop error checking, remove if/when appropriate
					i++; if(i==1000) { logger.log("Include range loop broken at end"); break; }
				} while (nextSibling);
			}
			...
			if(rantex.dom.isAncestorOf(startEl, r.endContainer)) {
				while(!nextSibling && r.endContainer.parentNode !== startEl) r.setEndAfter(r.endContainer);
			} else if(rantex.dom.isAncestorOf(endEl, r.startContainer)) {
		...
		surroundWithTag: function(tag) {
			...
			logger.error("Cannot Surround Content");
			return null;
		},
		...
		traverseThroughElements: function(sc, ec, callback) {
			if(rantex.dom.isAncestorOf(sc,ec)) {
			...
			} else if(rantex.dom.isAncestorOf(ec,sc)) {
				this.traverseFromElement({
					direction: 1, el: sc,
					childToAvoid: ec,
					fn: callback
				...
				var elementsToAvoid = [ec, rantex.dom.getCommonAncestor(sc,ec)];
				this.traverseFromElement({
					direction: 1, el: sc,
					childToAvoid: ec,
		...
		traverseFromElement: function(param) {
			...
			direction = param.direction,                      // Direction of iteration
			...
			logger.log('------- Starting Iteration', direction > 0 ? 'forward' : 'backward', 'from', el);
			while(curr !== elementsToAvoid && inArray(curr, elementsToAvoid) === -1) {

				if(curr) {
					if(isTNode(curr)) {
						range = rantex.createRange();
						...
						if(range.collapsed || (range.startContainer === range.endContainer && trim(range.startContainer.data) === "")) {
							el = curr;
					...
					if(rantex.dom.isAncestorOf(curr,childToAvoid)) {
						logger.log("------Found common parent", curr);
						return;
					}
					...
					if(el === elementsToAvoid || inArray(el, elementsToAvoid) !== -1) {
						logger.log("------Found repeated element", el);
						return;
					}
					curr = direction > 0 ? el.nextSibling : el.previousSibling;
				}
		...
		forInRange: function(callback) {
			...
			var r = this.r.cloneRange(),			// Hold on to our current range
				newRange = this.r.cloneRange(),		// Holds a new range (may be used later as instance's range)
			...
			if(span) {
				logger.log("There was only one element in range");
			...
			if(this.isTNode('s') && this.isTNode('e')) {
				...
				if(this.get('s') === this.get('e')) {
					logger.log("Same start and end container textnode");
				...
				while(next && isTNode(next)) {
					if(next === this.get('e')) {
						logger.log("Endpoints are siblings and only contain textnodes in between");
						span = this.surroundWithTag('span');
						callback.call(this, span);
						this.r.selectNodeContents(span);
						this.select();
			...
			console.log("not surroundable", this.isTNode('e'), r.endContainer);
			...
			if(this.isTNode('s')) {
				...
				if(this.offset().s === 0) callback.call(this, this.get('s'));
			}
			this.setRange(r);
			if(this.isTNode('e')) {console.log("end container text node")
				...
				if(this.offset().e === 1) callback.call(this, this.get('e'));
			}
