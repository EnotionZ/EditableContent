...
var CE = (function(){
	...
	$doc = $(document),
	$txa = $('<textarea/>').addClass('-ce-ta').appendTo(document.body).css({'width': 0, 'height': 0}),
	...
	ContentEditable = {
		init: function(param) {
			...
			this.wzRange.r.selectNodeContents(this.el[0]);
			this.wzRange.r.collapse();
		...
		enable: function() {
			var self = this;
			$txa.bind('keyup.'+this.id, function(e) { self.keyup(e); return false; });
			$doc.bind('keydown.'+this.id, function(e) { self.keydown(e); return false; });
		...
		isDesc: function(el) {
			if(this.el[0] === el || this.el.find(el).length) return true;
			return false;
		},
		...
		mousedown: function(e) {
			...
			$t = $(e.target), 
			_x = e.offsetX,
			_lh = parseInt($t.css('line-height'), 10);

			this.wzRange.setRange(rangy.getSelection().getRangeAt(0));
			if(e.target !== this.el[0]) {
				this.setBlinker(_x, $t.position().top, _lh);
				this.isMousedown = true;
				console.log("Setting Range", this.wzRange.r.startContainer, this.wzRange.r.startOffset);
			}
		},
		mouseup: function(e) {
			this.isMousedown = false;
